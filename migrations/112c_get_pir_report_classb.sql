-- =============================================================================
-- get_pir_report — Item 112 class-(b) pass (ADDITIVE, backward-compatible):
--   1. Delete the dead volusia_cama_permits block (computed v_permits/_count were
--      never emitted; permitFacts sub-RPC supersedes them).
--   2. land: add gopherTortoiseCoverage ('covered' | 'not_available') so the front
--      end can tell "no habitat nearby in a covered county" from "we hold no habitat
--      data for this county". Coverage is content-based (a Volusia gopher polygon's
--      interior point inside the parcel's county). Existing keys kept -> old FE unaffected.
--   3. Add top-level schoolsCoverage {field_status, who_can_answer}. `schools` (the
--      assigned array) is UNCHANGED. Non-Volusia -> not_available (the county school
--      assignment layer is Volusia-only; nearby-school amenities are a single, often
--      mislabelled row — not trustworthy to surface as "nearby schools").
-- Verified by per-key md5 diff on a Volusia parcel: only `land` changes and
-- `schoolsCoverage` is added; every other section (incl. `schools`) is identical.
-- =============================================================================
...
BEGIN
  ...
  -- Permits are resolved by get_parcel_permit_facts (see 'permitFacts'). The former inline
  -- volusia_cama_permits aggregation was dead code (computed, never emitted) — removed (Item 112).
