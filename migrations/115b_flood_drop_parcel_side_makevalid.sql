-- =============================================================================
-- Ruling 122 step 2 — drop get_parcel_flood_zone's parcel-side `v_geom := ST_MakeValid(v_geom)`.
-- resolve_parcel_geometry now returns valid geometry (parcels_staging + supplement both 0-invalid), so
-- re-repairing it here was repairing something already valid, on top of resolve's own (now removed) repair.
-- The two PER-ROW layer guards (CASE WHEN ST_IsValid(f.geom) ...) are DELIBERATELY KEPT: the served flood
-- layers still hold invalid geometry (dataset_content_audit: 138 of 279 layers) and are not yet repaired.
-- Removing those waits until each layer is provably clean (ruling 122 step 3). Body otherwise unchanged.
-- =============================================================================
