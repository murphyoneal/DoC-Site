-- =============================================================================
-- Item 80, step 3 (part a): loosen the derived-SFHA match to a PREFIX on A#/V#.
-- Re-deploys get_parcel_flood_zone (identical to 80b) with one change: when a
-- layer has no sfha flag and in_sfha is derived from the zone code, suffixed
-- legacy zones (A13F, "A2 F") must count as SFHA. The anchored '^(A|V)[0-9]{1,2}$'
-- missed them (Pasco) -> prefix '^(A|V)[0-9]'. Nothing non-SFHA starts with A#/V#,
-- so clean layers (Collier/Nassau/Walton/...) and the flag path (Volusia) are
-- unaffected. Verified: Volusia floodBlock md5 unchanged.
-- =============================================================================
