...
export function disclosuresView(input) {
  const title = 'Source limitations';
  // Accepts EITHER the legacy array (r.disclosures) or the envelope from
  // get_parcel_disclosures_facts (r.disclosuresFacts). The array path is unchanged.
  const env = input && !Array.isArray(input) && typeof input === 'object' ? input : null;
  const disclosures = env ? env.disclosures : input;
  ...
  if (items.length === 0) {
    // TWO DIFFERENT SILENCES, and only one of them is safe to render as nothing.
    // none_recorded: the defect registry IS held and WAS searched, and no recorded defect
    //   applies to this parcel. Showing no section is correct - the absence of a stated
    //   source limit is not a claim about anything.
    // not_available: no registry was consulted. Showing nothing there would imply the
    //   source has no known limitations, which is a claim we cannot support - so it gets
    //   said out loud instead.
    if (env && env.field_status === 'not_available') {
      return {
        mode: 'not_available', title, items: [],
        note: env.coverage_caveat
          || 'We could not consult our record of source limitations for this parcel. The absence of a stated limitation below is NOT a finding that the county source has none.',
      };
    }
    return { mode: 'none', title, items: [] };
  }
  return {
