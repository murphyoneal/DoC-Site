...
export function readPayload(payloadText) {
  ...
  // ASSERTION 4's evidence. A record that DECLARES a containment standard is subject to assertion 4;
  // one that does not is not. That is the discriminator, and it is why a real polygon containment
  // (a GWCA area, a flood zone) is untouched: those carry no containment_status, because containment
  // from a surveyed polygon is not in question. Only records that themselves say "this boundary can
  // only indicate" get held to it.
  const containment = [];   // { name, status }
  let root;
  ...
  const walk = (node, key) => {
    ...
    if (typeof node.containment_status === 'string') {
      containment.push({
        name: typeof node.name === 'string' ? node.name : null,
        status: node.containment_status.toLowerCase(),
      });
    }
  ...
  walk(root, null);
  return { absent, numbers: [...numbers], radiiM: [...radiiM], containment };
}
...
export function checkQualifiers(narration, payloadText) {
  ...
  const { absent, numbers, radiiM, containment } = readPayload(payloadText);
  const violations = [];
  ...
  // ─── ASSERTION 4 — a CONTAINMENT claim needs a record that can determine containment ─────────────
  // The first three assertions are all NUMERIC — a count, a zero, a radius — because D1-D4 were all
  // numeric and the guard inherited that shape. "within the Mountain Lake Estates Historic District"
  // contains no number, so nothing fired, and a claim with legal consequence went out flat over a record
  // whose boundary is a nomination bounding BOX that cannot determine containment (item 253).
  //
  // THE PRECONDITION THAT MADE THIS BUILDABLE: until migration 138a/b the payload contradicted itself —
  // one block reported membership, another refused to — so a guard here would have had to pick a block,
  // encoding whichever one its author happened to read. Now containment_status says which standard
  // applies, in data, so the guard reads the record instead of guessing.
  //
  // HEDGED LANGUAGE IS THE POINT, NOT AN EXCEPTION. Reporting an indication AS an indication is the
  // specified behaviour and the report page already words it that way. A guard that flags the correct
  // output gets muted within a week — that is the tourniquet lesson, where the caveat added by 125a armed
  // the guard against its own honest payload.
  const CONTAINMENT_VERB = /\b(?:within|inside|contained in|located in|sits in|falls (?:in|inside|within)|part of)\b/i;
  const HEDGED = /\b(?:indicat\w*|requir\w*\s+confirmation|not a determination|to confirm|confirm against|unconfirmed|nomination[- ]derived|not survey|bounding (?:box|coordinate)|appears?|may be|possibl\w*|treat\w*\s+as|unclear|uncertain|whether|cannot be determined|not determinable)\b/i;
  // A NEGATIVE CLAIM IS NOT A CONTAINMENT CLAIM. "The parcel does not fall within X" asserts the opposite
  // of containment and must pass — flagging it would mute the guard on exactly the honest wording we want.
  // Checked in the window immediately BEFORE the verb, so a negation elsewhere in a long sentence cannot
  // launder an assertion made later in it.
  const NEGATED_BEFORE = /\b(?:not|no|never|n't|outside|excluded)\b[^.;]{0,24}$/i;

  for (const rec of containment) {
    if (rec.status === 'containment') continue;   // determinable: a flat claim is licensed
    for (const sen of sentences) {
      if (!CONTAINMENT_VERB.test(sen)) continue;
      // Match either the record by NAME, or a generic district claim when the payload holds only
      // indications — Roz named the district; the page's wording does not name one at all.
      const named = rec.name && sen.toLowerCase().includes(rec.name.toLowerCase());
      const generic = /historic district/i.test(sen);
      if (!named && !generic) continue;
      if (HEDGED.test(sen)) continue;             // an indication reported AS an indication is correct
      const vm = CONTAINMENT_VERB.exec(sen);
      if (vm && NEGATED_BEFORE.test(sen.slice(0, vm.index))) continue;   // a denial of containment
      violations.push({
        assertion: 'containment_asserted_over_indication',
        field: rec.name, status: rec.status,
        detail: 'containment_status is ' + rec.status + ' — this boundary can indicate membership, not determine it, so the claim needs hedging or a determinable record',
        sentence: sen.trim().slice(0, 240),
      });
      break;
    }
  }

  return { violations, absentFields: absent.length, payloadNumbers: numbers.length,
           payloadRadii: radiiM.length, containmentRecords: containment.length };
}
