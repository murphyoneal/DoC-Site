...
import { renderFact, renderMarineBlock, renderValuesBlock, renderCensusBlock, renderOwnersBlock, renderTransactionsBlock, renderPermitsBlock, renderZoningBlock, renderSinkholeBlock, renderRestrictionsBlock, renderBrownfieldBlock, NULL_STRINGS } from './fact-render.mjs';
...
test('ground_elevation present: renders FEET (nearest foot) + the two-clause caveat, NO BFE difference', () => {
  const v = renderFact({ predicate: 'ground_elevation', value: 35, field_status: 'present',
    source: 'USGS EPQS', source_tier: 'government_derived', vertical_datum: 'NAVD88',
    caveat: 'Bare-earth ground at one interior point on the parcel. Not a surveyed lowest-floor elevation and not an elevation certificate.',
    note: 'Determining elevation relative to a base flood elevation requires a surveyed elevation certificate from a Florida-licensed surveyor.',
    corroborators: [], contradictors: [], open_questions: [] });
  assert.equal(v.label, '35 ft');                       // feet, nearest foot — never a metre, never a decimal
  assert.equal(v.hasValue, true);
  assert.ok(!/\bBFE\b|base flood|above\b/i.test(v.label), 'must NOT compute a ground-vs-BFE difference');
  assert.ok(noConfidenceScore(v));
});
...
const VF_MARION = {
  ...
  // SPEC_FEAT_VAL is a real NAL column — a SOURCEABLE fact (government_derived, its own as_of), the $45.6M that
  // the improvement subtraction depends on.
  special_feature_value: vf('special_feature_value', 45632549, 'marion_nal_dor_source', 'government_derived', '2025 roll (NAL — DOR annual snapshot)'),
  // improvement is COMPUTED on the NAL branch — it declares its inputs so a reader sees where the number came from.
  improvement_value: { ...vf('improvement_value', 90948950, 'marion_nal_dor_source', 'government_derived', '2025 roll (NAL — DOR annual snapshot)'),
    derivation: { computed: true, formula: 'just_value − land_value − special_feature_value', inputs: ['just_value', 'land_value', 'special_feature_value'] } },
...
test('values: special-feature value is a sourceable FACT ($45.6M), and improvement NAMES it as an input', () => {
  const b = renderValuesBlock(VF_MARION);
  const spec = b.fields.find((f) => f.key === 'specialFeatureValue');
  assert.ok(spec, 'special_feature_value must be a rendered field, not a bare number');
  assert.equal(spec.rendered.label, '$45,632,549');                         // sourceable, not derived
  assert.equal(spec.rendered.provenance.tier, 'government_derived');
  assert.equal(spec.rendered.provenance.as_of, '2025 roll (NAL — DOR annual snapshot)');  // its OWN roll year
  const imp = b.fields.find((f) => f.key === 'improvementValue');
  assert.ok(imp.rendered.provenance.derivation, 'improvement must declare it is computed');
  assert.match(imp.rendered.provenance.derivation.formula, /special_feature_value/);       // names the input
  assert.ok(!imp.rendered.provenance.derivation.our_inference, 'assessor arithmetic is NOT our inference');
});
...
// A value we could NOT evidence as a real category (Charlotte "City", Clay "MUNI", Seminole "CITY",
// Miami-Dade "NONE") — a suspected jurisdiction placeholder. It is shown VERBATIM (never hidden, never
// re-mapped) AND carries the honest "we couldn't confirm this" caveat. Distinct from municipal_not_held:
// there the county named a city; here we have neither a city name nor a confirming description.
test('zoning: an unconfirmed flagged value shows the code verbatim AND says so in the output', () => {
  const ZF_FLAGGED = { field_status: 'present',
    zoning: { predicate: 'zoning', value: 'CITY', description: null, field_status: 'present',
      jurisdiction: 'Seminole County', jurisdiction_level: 'county', source: 'seminole_zoning',
      source_tier: 'government_derived',
      unconfirmed_note: 'This value could not be confirmed as a real category from the data — it may be a jurisdiction placeholder (an incorporated/unzoned marker) rather than a designation. Shown exactly as recorded; treat with caution and verify against the land development code.' },
    future_land_use: null, relationship: null, coverage_note: null };
  const b = renderZoningBlock(ZF_FLAGGED);
  assert.equal(b.zoning.code, 'CITY');                          // the value is shown verbatim, not suppressed
  assert.ok(b.zoning.unconfirmedNote, 'the unconfirmed caveat must surface in the output');
  assert.ok(/could not be confirmed|treat with caution/i.test(b.zoning.unconfirmedNote));
  assert.ok(noConfidenceScore(b), 'a caveat is not a confidence score');
});
...
// Land-use restrictions (get_parcel_restrictions) — a state_regulatory CONSTRAINT, containment not distance.
const RESTRICTIONS_GWCA = [{
  field: 'gwca_restriction', category: 'restriction', ladm_class: 'LA_Restriction',
  value: 'within a delineated Groundwater Contamination Area (Ch. 62-524)', field_status: 'present',
  as_of: 'FDEP WRM_UIC', source: 'gwca_parcel_match', authority: 'FDEP', relation: 'contains',
  wells_on_parcel: 0, wells_in_gwca: 0,
  coverage_caveat: 'No mapped well on this parcel. Ch. 62-524 restricts NEW potable wells inside the GWCA; a property on municipal/public water is unaffected in practice.',
}];
test('restrictions: a GWCA finding renders as a state_regulatory constraint with the well caveat verbatim', () => {
  const b = renderRestrictionsBlock(RESTRICTIONS_GWCA);
  assert.equal(b.established, true);
  assert.equal(b.items.length, 1);
  assert.equal(b.items[0].tier, 'state_regulatory');            // like the flood mandate, not area context
  assert.match(b.items[0].value, /Ch\. 62-524/);                 // the regulatory chapter, verbatim
  assert.match(b.items[0].caveat, /municipal\/public water|NEW potable wells/); // the resolver's nuance, unaltered
  assert.equal(b.items[0].relation, 'contains');                 // containment stated, not softened
  assert.ok(noConfidenceScore(b));
});
test('restrictions: an empty result is an HONEST absence — never a clearance, names historic use', () => {
  const b = renderRestrictionsBlock([]);
  assert.equal(b.established, false);
  assert.equal(b.items.length, 0);
  assert.ok(b.absenceNote, 'absence must be stated');
  assert.ok(/not a clearance|never .?clean.?|not in a mapped restriction/i.test(b.absenceNote), 'must refuse to imply clean');
  assert.ok(/grove|pit|vat|discovered and regulated|historic/i.test(b.absenceNote), 'must name the historic-use gap');
  assert.ok(noConfidenceScore(b));
});
test('restrictions: null/garbage input is the honest-absence path, not a crash', () => {
  for (const bad of [null, undefined, {}, 'x', 0]) {
    const b = renderRestrictionsBlock(bad);
    assert.equal(b.established, false);
    assert.ok(b.absenceNote);
  }
});

// The envelope path (get_parcel_restrictions_facts -> r.landRestrictionsFacts). The bare-array
// path above is untouched; these assert the ADDITION. The defect being closed is subtle: the
// legacy note was already honest ("never clean") but named only the CONTAMINATION gap, so on a
// platted airpark it was a true statement about a question the buyer was not asking.
const RESTRICTIONS_ENVELOPE_EMPTY = {
  field: 'restrictions',
  field_status: 'not_available',
  restrictions: [],
  searched: ['groundwater_contamination_area', 'institutional_control', 'well_restriction'],
  not_searched: ['recorded_covenants_and_deed_restrictions', 'plat_restrictions', 'hoa_declarations'],
};
test('restrictions: the envelope APPENDS the unsearched classes and never replaces the legacy note', () => {
  const legacy = renderRestrictionsBlock([]);
  const b = renderRestrictionsBlock(RESTRICTIONS_ENVELOPE_EMPTY);
  assert.equal(b.established, false);
  assert.equal(b.items.length, 0);
  // every guarantee the legacy note carried must survive verbatim, as a prefix
  assert.ok(b.absenceNote.startsWith(legacy.absenceNote), 'legacy sentence must be preserved intact');
  assert.ok(/not a clearance|never .?clean.?|not in a mapped restriction/i.test(b.absenceNote));
  assert.ok(/grove|pit|vat|discovered and regulated|historic/i.test(b.absenceNote));
  // and the class that actually binds a platted parcel must now be named
  assert.ok(/covenant/i.test(b.absenceNote), 'must name recorded covenants');
  assert.ok(/plat restrictions/i.test(b.absenceNote), 'must name plat restrictions');
  assert.ok(/HOA declarations/i.test(b.absenceNote), 'must name HOA declarations');
  assert.ok(/NOT searched/i.test(b.absenceNote), 'must say these were not searched, not that they are absent');
  assert.ok(noConfidenceScore(b));
});
test('restrictions: an envelope carrying items renders them exactly as the bare array does', () => {
  const viaArray = renderRestrictionsBlock(RESTRICTIONS_GWCA);
  const viaEnvelope = renderRestrictionsBlock({ field_status: 'present', restrictions: RESTRICTIONS_GWCA });
  assert.equal(viaEnvelope.established, true);
  assert.deepEqual(viaEnvelope.items, viaArray.items, 'envelope and array must render identically');
  assert.equal(viaEnvelope.absenceNote, null);
});

// Brownfield — a FINDING (statewide), inside=containment / sites=proximity, sentinel remediation, honest absence.
const BF_INSIDE = { field_status: 'present', source_tier: 'government_derived',
  inside_area: { name: 'Perrine Area', resolution_number: '99-767', resolution_date: 'July 13, 1999', acreage_ac: 2689.6, county: 'Miami-Dade', source: 'fdep_brownfield_areas', relation: 'contains' },
  nearest_area: null,
  nearby_sites: { count_within_1mi: 6, nearest: { name: 'Tucker Tower Green Reuse Site', distance_ft: 1257,
    remediation_status: null, remediation_status_note: 'The FDEP record does not state a remediation status for this site — not recorded, not "clean".', site_address: '17800 S Dixie Hwy', verification_date: null } },
  note: 'A brownfield is a site FDEP has designated for redevelopment because real or perceived contamination complicates reuse.' };
test('brownfield: inside a designated AREA is the finding; sites are area context; both surface', () => {
  const b = renderBrownfieldBlock(BF_INSIDE);
  assert.equal(b.established, true);
  assert.equal(b.insideArea.name, 'Perrine Area');
  assert.equal(b.insideArea.resolutionNumber, '99-767');           // the county ordinance, verbatim
  assert.equal(b.sites.countWithin1mi, 6);
  assert.equal(b.sites.nearest.distanceFt, 1257);                  // pinpoint distance, not containment
  assert.ok(noConfidenceScore(b));
});
test('brownfield: a null remediation status shows WHY (not recorded != clean), never a blank status', () => {
  const b = renderBrownfieldBlock(BF_INSIDE);
  assert.equal(b.sites.nearest.remediationStatus, null);
  assert.ok(/not recorded|not .?clean.?/i.test(b.sites.nearest.remediationStatusNote), 'the sentinel must render its meaning');
});
test('brownfield: none_nearby is an HONEST absence — not a clearance, names the voluntary-designation gap', () => {
  const b = renderBrownfieldBlock({ field_status: 'none_nearby', source_tier: 'government_derived',
    coverage_note: 'This parcel is not inside — and has no FDEP brownfield site within a mile of — any designated brownfield area we hold. Not a clearance: a brownfield designation marks a site voluntarily entered into FDEP\'s cleanup program, so prior or suspected contamination can exist on parcels that were never designated.' });
  assert.equal(b.established, false);
  assert.ok(/not a clearance|never designated|voluntar/i.test(b.coverageNote), 'absence must refuse to imply clean');
  assert.ok(noConfidenceScore(b));
});
test('brownfield: null/garbage input is the honest-absence path, not a crash', () => {
  for (const bad of [null, undefined, {}, 'x', 0]) {
    const b = renderBrownfieldBlock(bad);
    assert.equal(b.established, false);
    assert.ok(b.coverageNote);
  }
});
