...
export const NULL_STRINGS = {
  ...
  special_feature_value: 'Special-feature value not recorded by the county',
...
// NOTATION, NOT ALTERATION (ruling 505 part 2). Where a source publishes in metric, the getter serves
// the published figure plus sibling keys — value / units / value_us — and the RENDERER composes the
// pair. The getter never emits a composed string: every units failure so far happened inside composed
// prose, and a structural sibling leaves the narrator nothing to compute.
// Guarded on both siblings being present, so records without notation keep their existing label.
function notationLabel(rec) {
  if (!rec || rec.units == null || rec.value_us == null) return null;
  return `${rec.value} ${rec.units} (${rec.value_us})`;
}

function valueLabel(predicate, value, rec) {
  const notated = notationLabel(rec);
  if (notated) return notated;
  ...
  switch (predicate) {
    ...
    // FALLBACK ONLY. Since migration 125a the elevation getter serves notation siblings (value 3.55 /
    // units 'm' / value_us '12 ft') and notationLabel above renders the pair, because USGS 3DEP
    // publishes in metres and NAVD88 is a metric datum. This arm survives for any record that still
    // arrives without the siblings, and it assumes value is ALREADY IN FEET and already rounded — a
    // metres value reaching here renders 3.4x low. Never a decimal: a single-point 3DEP sample carries
    // several-feet error on a fifth of parcels, so 0.1 ft would imply an inch we do not have. The
    // two-clause caveat travels with the value in both arms.
    case 'ground_elevation':     return `${n(value)} ft`;
    case 'just_value':
    case 'assessed_value':
    case 'land_value':
    case 'special_feature_value':
...
export function renderFact(rec) {
  ...
  // THREE COVERAGE STATES, NEVER TWO. not_available means WE hold nothing; not_recorded means the
  // COUNTY recorded nothing. Falling through to NULL_STRINGS below would print "not recorded by the
  // county" about a gap that is ours — a claim about the county we have no basis for. Scoped to
  // records carrying their own coverage note, so nothing that relies on the NULL_STRINGS text moves.
  if (rec.field_status === 'not_available' && rec.note) {
    return { ...empty, label: rec.note };
  }
  ...
  return {
    label: valueLabel(rec.predicate, rec.value, rec),
    hasValue: true,
    provenance,
    corroboration,
...
export function renderFloodBlock(block) {
  ...
  // RULING 251 — THREE STATES, NOT TWO. Zone D means FEMA performed NO analysis: it is neither in
  // nor out of the SFHA. Testing only `=== true` collapses undetermined into "Not in a Special
  // Flood Hazard Area", which is the St Pete false-clearance failure this block warns about.
  // It is also DISTINCT from not_established: that is a gap in OUR data, this is a gap in FEMA's.
  const undetermined = d.field_status === 'undetermined' || (!notEstablished && d.value == null);
  const inSfha = d.value === true;
  const determination = {
    established: !notEstablished,
    undetermined,
    inSfha: (notEstablished || undetermined) ? null : inSfha,
    label: notEstablished
      ? 'Flood determination not established — a coverage gap about our data, not the parcel'
      : undetermined
        ? 'Undetermined — FEMA has performed no flood analysis here (Zone D). This is not a clearance.'
...
const VALUE_FIELDS = [
  ...
  ['specialFeatureValue', 'special_feature_value'],  // now a sourceable fact (SPEC_FEAT_VAL), no longer a flat key
  ['improvementValue',    'improvement_value'],       // carries a derivation naming just−land−special on the NAL branch
];
...
export function renderOwnersBlock(of) {
  ...
  const owners = (Array.isArray(of.owners) ? of.owners : []).map((rec) => {
    ...
    return {
      ...
      // RULING 267 — a named party at a ZERO share is NOT a 0% owner. pctOwn is suppressed so it
      // cannot render as "0% as recorded"; shareStatus carries the fact and shareNote the caveat.
      // A lienholder, remainderman, trustee or released spouse looks identical here and the column
      // cannot say which — so we state what the county recorded and infer nothing.
      pctOwn: rec.share_status === 'zero_recorded' ? null : (rec.pct_own ?? null),
      shareStatus: rec.share_status ?? null,
      shareNote: rec.share_note ?? null,
  ...
  // RULING 266 — shareCheck is THREE STATES and only TIC can produce true/false.
  // value === false is a FINDING and gets its own visible line.
  // value == null renders NOTHING — not a dash, not "unknown". A null that renders as
  // anything is the Zone D bug: the reader cannot tell "not applicable" from "checked and fine".
  const sc = of.share_check || null;
  const shareCheck = sc && sc.value != null
    ? { value: sc.value === true, failed: sc.value === false, reason: sc.reason ?? null, note: sc.note ?? null }
    : null;
  return {
    established: true,
    ownerCount: oc ? { value: oc.value ?? null, note: oc.note ?? null, asOf: oc.as_of ?? null, tier: oc.source_tier ?? null } : null,
    owners,
    // the tenancy FORM renders beside the owners ALWAYS — TE vs TIC vs LE is a different right,
    // and it is what a buyer needs: a creditor of one TE spouse generally cannot reach the
    // property, while any one TIC holder can force a partition sale.
    tenancy: of.tenancy
      ? { form: of.tenancy.form ?? null, mixed: of.tenancy.mixed === true,
          formsRecorded: of.tenancy.forms_recorded ?? null, note: of.tenancy.note ?? null }
      : null,
    shareCheck,
...
const RESTRICTION_NOT_SEARCHED_LABEL = {
  recorded_covenants_and_deed_restrictions: 'recorded covenants and deed restrictions',
  plat_restrictions: 'plat restrictions',
  hoa_declarations: 'HOA declarations',
  private_easements: 'private easements',
  source_water_protection: 'source-water / wellhead protection',
};
// Accepts EITHER the legacy bare array (r.landRestrictions) or the envelope from
// get_parcel_restrictions_facts (r.landRestrictionsFacts). The array path is unchanged, so
// every existing caller and every existing test behaves exactly as before.
export function renderRestrictionsBlock(input) {
  const env = input && !Array.isArray(input) && typeof input === 'object' ? input : null;
  const arr = env ? (Array.isArray(env.restrictions) ? env.restrictions : []) : input;
  ...
  if (!Array.isArray(arr) || arr.length === 0) {
    // APPEND, never replace. The existing sentence is asserted by fact-render.test.mjs and is
    // correct as far as it goes - but it names only the CONTAMINATION gap. On a platted airpark
    // the binding class is recorded covenants, and it never mentioned them: a true statement
    // about a question the buyer was not asking. The envelope's own not_searched list is the
    // fix, because a hardcoded caveat names the gap its author had in mind while not_searched
    // names the gap THIS parcel actually has.
    const gaps = env && Array.isArray(env.not_searched) ? env.not_searched : [];
    const named = gaps.map((k) => RESTRICTION_NOT_SEARCHED_LABEL[k] ?? String(k).replace(/_/g, ' '));
    const extra = named.length
      ? ` Separately, these restriction classes were NOT searched, because we do not hold them for this parcel: ${named.join('; ')}. On a platted subdivision, an airpark, or any community with a homeowners association, those recorded instruments are usually the binding restriction — this section cannot speak to them at all.`
      : '';
    return { established: false, items: [], absenceNote: absenceNote + extra };
  }
