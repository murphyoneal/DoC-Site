...
function valueLabel(predicate, value) {
  ...
  switch (predicate) {
    ...
    // Ground elevation renders in FEET, nearest foot (the resolver already rounds). Never a decimal — a
    // single-point 3DEP sample carries several-feet error on a fifth of parcels (the sampling defect), so
    // a 0.1 ft figure would imply an inch of accuracy we do not have. The two-clause caveat travels with it.
