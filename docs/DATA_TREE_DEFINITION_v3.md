...
**Version 3 · 2026-07-27.** Supersedes v2 (same date), which superseded v1.
...
**Changes in v3:** §11 orphan resolution — six of seven resolved by testing · §12 the negative-control finding on `sjc_plat_index` · §3 join topology corrected · §14 two classification errors of mine, recorded.

*v2 added: parcel-to-parcel relation keys, the `comm_` correction, the not-a-key register.*
...
## 3. Join topology — corrected in v3
...
| **J12 indirect key** | **5** | 531,061 | joins via another table — §11 |
...
**v2 reported 5 no-path and 2 broken loads. Both figures were wrong.** See §11.
...
## 6. Parcel-to-parcel relation keys

Recorded in `parcel_relation_key`.
...
This is the only link from a unit to its building. The association's 11 permits — including **concrete restoration 2022, $589,750** — reach a unit through this key and no other.
...
**Decisive:** neither the Oceans Atrium association parcel `4988688` nor unit `4990518` appears in `comm_bldg`. If `COMM` meant common elements, the association would be its first row.
...
## 11. Orphan resolution — NEW IN v3

v2 listed 5 no-path and 2 broken loads. **Six of seven resolved by testing.**

| table | rows | v2 said | tested | actual |
|---|---|---|---|---|
...
| `clay_flood_zones` | 1,239 | no path | **1,236 of 1,239** match `fema_flood_zones.fld_ar_id` | indirect key join, 99.8% |
...
**The two "broken loads" were complete by design.** A one-column table of `oid` values is an ID list carrying a boolean meaning — the single column is the entire point. My classifier treated column count as a quality signal, which it isn't.

**And `agent_license_status` was never an orphan.** It describes people, not land. The classifier only looked for parcel routes, so anything outside the parcel domain fell through.

**One genuine orphan in 1,226 tables.**

---

## 12. `sjc_plat_index` — the negative control — NEW IN v3

**Not corrupted.** `objectid` runs 1 to 2,709 with **zero gaps**. 2,709 rows, 2,709 distinct `sub_number`. Real plat names throughout: *Treaty Oaks Phase 1*, *Celestina Phase 4C*, *Palencia North Phase III A-2*, *Julington Lakes Phase 1*. The pull is complete.
...
**The STRAP hypothesis, tested and rejected.** STRAP is 10 digits: a 6-digit prefix plus a 4-digit lot (`PIN` shows the split: `000020 0010`). 232 of 2,709 `sub_number` values match a prefix — 8.6%.

**First test was invalid.** Four matched prefixes were checked for spatial clustering and all four clustered tightly. But those prefixes held 1, 1, 2 and 5 parcels — **a single parcel is always clustered**. The test could not fail.
...
**Clustering proves nothing.** Every STRAP prefix is a geographic grouping in the county's own numbering. The 8.6% overlap is two numeric ranges intersecting.

**No plat polygon layer is published.** St Johns serves parcels, zoning, future land use, address sites and a land-use boundary. The Clerk of Circuit Court holds the authoritative recorded plats under Ch. 177 F.S. with no bulk route and no predictable scan URL.
...
## 14. Classification errors in this survey — NEW IN v3
...
**And a test that could not fail.** The first STRAP clustering check used samples of 1 to 5 parcels and confirmed the hypothesis. A negative control on adequately sized samples reversed it completely — 40 of 40 unmatched prefixes clustered as tightly as the matched ones.

**That is `DATA_JOIN_FINDINGS` §1 test 3, and it is the test people skip.** Without it, test 2 produces a number with no meaning.
...
**And a match rate is not evidence.** Run the negative control.
...
| `nr_failures` | **77** | logged failures with phase, table, detail |
