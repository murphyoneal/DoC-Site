...
7. **Fabrication is the default assumption for report-source tables now.** Three examined, **two entirely fabricated** (`property_environmental` 13/15, `property_hazard_risk` 25/25). Don't audit `v_si`, `v_cen`, `properties`, and the `get_site_intelligence` sources one at a time — build one generalized predicate: for every column the report renders, fail if `count(DISTINCT) <= 1` across the table (a DB function iterating the rendered columns via `information_schema`, called as `SELECT … AS ok`). It catches the next fabricated table automatically instead of waiting for someone to notice. `get_site_intelligence` reads unchecked tables and is still in the Roz path — prioritise it.
