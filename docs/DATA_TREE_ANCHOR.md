...
7. **Report-source integrity gate — provenance primary, cardinality backstop.** Base rate: of four report data sources, **two were entirely fabricated** (`property_environmental`, `property_hazard_risk` — both dropped §7), one real (`v_si`, verified computed-at-query-time against sourced registers). The durable **primary** gate is provenance: no field renders in a paid report unless its backing table has a recorded source in `table_inventory` — it fails closed even on *plausibly-varied* fabrication, which cardinality cannot. **Blocker:** `source_url` is recorded for only ~3 of 15 report-source tables (null on `parcel_elevations`, `traffic_aadt`, `census_*`, `hydrology_waterbodies`, `properties`, …), so the gate would fire on real data today — **backfill provenance first.** Cardinality is the **backstop**: threshold pinned at `<= 1` *forever* (`fema_flood_zones.fld_zone` legitimately has 6 codes; low cardinality ≠ fabrication), and it detects only the *constant* failure mode — a table seeded with plausible per-parcel randomness passes it clean (the same residual as any self-consistent check; do not read a green as proof of authenticity). `get_site_intelligence` reads unaudited tables and is still in the Roz path — sweep it under whichever gate lands first.
