...
| 2026-07-30 | Registry-consulted-by-harness — **DEF-005 detection EXTENDED** from `county_layer_registry.srid` (blind to unregistered tables like FUDS) to a row-probe via `detect_srid_metadata_lie()`, folding my parallel detector INTO DEF-005 rather than maintaining a fifth map; mapped to `harness_predicate='geometry-srid-metadata-not-lying'`. New column `data_defect_registry.harness_predicate` lets each defect declare how it is mechanically watched (DEF-003→fragment predicate, DEF-024→provenance ratchet). New predicate `active-repair-defects-are-harness-tracked` (manifest → 16): an active `repair` defect with a globally-runnable detection and no `harness_predicate` fails the build — the guard that would have stopped DEF-005 sitting active 6 days | RED on **DEF-017 / DEF-020 / DEF-021** (untracked repair defects). FUDS coverage measured: **183,085** parcels intersect a boundary, **61,936** a munitions area — served correctly by existing containment (incident retracted, §9). Sanity: **422/500** sampled intersecting parcels are truly *contained* (point-in-polygon), not bbox overlap — so 183k is real, do not re-litigate | revert: drop column + predicate; restore DEF-005 detection |
| 2026-07-30 | `get_parcel_disclosures(p_co_no,p_parcel_id)` + wired into `get_pir_report` (new `disclosures` key) — the registry becomes a **product feature**. Surfaces active **source/disclose** defect text as a *stated source limit* (`kind='source_limit'`), distinct from a `not_available` coverage gap. County-scoped via new `data_defect_registry.applies_to_counties` (resolved by **lookup, not a literal** — Volusia's 8.8% never leaks elsewhere, the `areaRepetitiveLoss` guard); DEF-003 by per-parcel fragmentation. `disposition='disclose'` ONLY — an `ours/repair` defect is never a property fact. DEF-022 (FUDS) stays in `get_parcel_env_findings` (no double-emit); DEF-023 (marine) pending a per-parcel check | Volusia `744901030061` → `disclosures:[DEF-019]`; St.Johns fragment `65/0251700001` → `[DEF-003]`, **not** DEF-019; Orange → `[]`. Report smoke: `disclosures` is an array, DEF-019 present | revert: restore `RETURN v_out;`; drop function + column |
| 2026-07-30 | **Fact-index deterministic renderer — STEP 0 of the accepted route change** (severity-ordered, read-path-scoped, moat-first; the mechanical output guard ships FIRST, not fifth — fabrication is the worst §5 failure). `get_parcel_marine_fact()` returns a fact record (subject·predicate·value·source·source_tier·as_of·corroborators·contradictors·open_questions·note); `lib/fact-render.mjs` renders it **deterministically — the model may narrate but never originate a value**. Three rules, each tied to a §5 failure: absent value → a FIXED per-predicate string ("Footprint not recorded by the county") — kills the ±0.96 ft fabrication (a value the model composed over an absent field); independence rendered by STATUS, `unverified` → "not established" never "independent"; NO confidence score. Independence layer: new tier `county_assessor_record`; new `partially_informed_by` relation (CAMA↔permits — measured partial, NOT circular: `|YRBLT−permit yr|` decays 60/23/7/5/4% over 0..4yr, and 6,097 build years have no permit); `roz_source_relationship` → {independent, status ∈ derived/partial/unverified/independent_verified, qualifier}; `roz_source_ancestors` now follows `derives_from` ONLY so a weak edge can't collapse independence | real records: 2001178 dock 2,856 sqft (single-source), built 1998 (3 permits, partially independent), 2001160 built 2011 (open question); `fact-render.test.mjs` 6 controls green | revert: drop 3 functions + 2 lib files |
| 2026-07-30 | **Marine block wired through the fact index — the moat, rendered** + primitive renamed `roz_sources_independent`→`roz_sources_lineage_disjoint` (it tests lineage disjointness, NOT independence — a disjoint pair can be only partially independent; `status` is the honest answer, and a direct caller of the boolean no longer gets a small lie). `get_parcel_marine_fact` now covers 7 predicates; `get_parcel_marine_block` assembles improvements×predicates + pulls the material caveat from **DEF-023 (registry, not prose)**; `renderMarineBlock` renders deterministically. Four holds: derived values declare themselves (`pct_depreciated` carries {method:`1-RCNLD/RCN`, inputs} on a `derived` tier); **OUR inference** (`service_life_vs_age`) is `source=dop_analysis`, `tier=analysis_inference` — never the assessor's authority (dock ~30yr / seawall ~50yr from 12,784 records; boathouse/lift → "not derived"); null is **theoretical** for marine (AREA/GRADE/RCN/RCNLD never null across 16,460 rows) | **1212 S Riverside (PARID 6473111)**: 4 improvements, **3 open questions** (2009 dock/boathouse/lift, no permit 2008–2010), the 2010 seawall's only date-match a 2011 $706,819 no-description permit → "partially independent". 10 render controls green | revert: drop the 3 marine functions + rename back |
| 2026-07-30 | **Marine block rendered on the report page — the moat, now visible to a buyer.** Replaced the `marineView` prose with the fact-index render: `get_parcel_marine_block_by_parcel` (DOR→CAMA `PARID` via `volusia_cama_parcel.DORID`; coverage-aware — non-Volusia = `not_available`) wired into `get_pir_report` payload (`r.marineBlock`) → `renderMarineBlock` on `page.tsx`. The **cross-examination headline leads** (block-level `open_questions` roll-up aggregated in the block fn, per-fact questions kept as source); three provenance tiers stay visually distinct (`TierBadge`: county / derived / **OUR estimate** accent, so our 30yr dock curve never reads as the assessor's); "partially independent" carries a specific gloss (large permit, no matching description). Non-Volusia = coverage gap, never "no dock" | `report-coverage.test.mjs` 16 controls (+3 marine: headline / no-manufacture+gloss / gap-clean); typecheck clean; `get_pir_report(74,'744403020120').marineBlock` = present / 4 improvements / "3 of 4" headline. **Visual confirm pending deploy** (local env key drift) | revert: restore the `marineView` IIFE + drop `marineBlock` from the payload |
| 2026-07-30 | **Flood through the fact index** — the legal-consequence block. New tier **`federal_regulatory`** (FEMA *binds* the property, it doesn't describe it — regulatory vs the assessor's observation; the distinction that makes the question budget defensible). `get_parcel_flood_block` reshapes `get_parcel_flood_zone` so the **SFHA determination (`in_sfha`) is the subject/headline** (it triggers the federal insurance mandate); zone/BFE/% are supporting. **Coverage gap = a fact about OUR knowledge** ("Not established — no county NFHL layer is held here; a statement about our coverage, NOT the parcel; not a finding it's outside a flood zone") — the St Pete substitution is now structurally impossible. Vertical **datum (NAVD88) surfaced** (the one provenance we hold); elevation-above-BFE **withheld with the asymmetry made visible** (`parcel_elevations` has no datum), not silently omitted. FEMA singular authority → `corroborators:[]`. `renderFloodBlock` + wired into `get_pir_report` (`r.floodBlock`) | St Pete `in_sfha` true / NAVD88; **Marion (uncurated) → `not_established`** (4 uncurated: Broward=16 / Lee=46 / Leon=47 / Marion=52). `report-coverage.test.mjs` **18 controls** (+2 flood); typecheck clean | revert: restore the `floodView` IIFE + drop `floodBlock` |
...
14. **Ask the Volusia appraiser: is `YRBLT` set from building permits?** The CAMA↔permits `partially_informed_by` edge (fact index) rests on a strong *inference* (decaying offset + 6,097 permit-less build years) that they are partially independent. A five-minute question through the Volusia contact converts the inference into a recorded fact and either confirms `partial` or downgrades toward `derived` — which changes how the 9,704 "corroborated" marine improvements may be described. Until answered, corroboration renders with the qualifier, never as full independence.
15. **Cardinality backstop needs rendered-column scoping** (not built). A table-level `n_distinct ∈ {0,1}` scan over the 32 report-source tables fires on **102 columns, ~all legitimate** (`state='FL'`, CAMA `TAXYR` single-year loads, null `expand_*`, GIS `created_user`) — the cry-wolf case. It is only meaningful over the columns the report actually **renders**, which needs a field→backing-column manifest. Build that, keep the threshold at `<= 1` (never raise), and note it catches only the *constant* failure mode: a table seeded with plausible per-parcel randomness passes it clean (same residual as any self-consistent check — a green is not proof of authenticity). `get_site_intelligence` reads unaudited tables and is live in the Roz path.
...
> **Standing rule (generalises past one table): a redundancy claim requires a column diff, not a row count.** `fl_cadastral_dor_statewide` reached "DROP it" on plausible row-count reasoning and it took a 14-column comparison to refute — it is a superset. The next candidate for deletion will look equally dead. Never retire a table (or call it redundant) without diffing columns against the table it supposedly duplicates.

> **Standing rule: a detector that produces a surprising count must be validated against a known-good case before its output is believed.** Three flood-extent detectors in one session — single-sample point-in-county (flagged 7, all regional extracts), interior-point-coverage (flagged 16, including `leon_flood_zones_firm` which had been *proven serving Leon minutes earlier*), boundary-intersection (0, the truth). A check that flags a case you just verified working is the cheapest possible signal that the **check** is wrong, not the data — the negative-control rule turned on your own detector. A single spatial sample is the millisecond-benchmark mistake in a spatial costume.

> **Standing rule: never repair geometry per query — validate ONCE at ingest. And profile to the single bottleneck before fixing a list of suspects.** The Ocala report timed out twice in production (25s budget). The maintainer listed four contributors; profiling the sub-resolvers showed ONE of them was 26s of the 27.7s: `get_parcel_flood_zone` ran `ST_MakeValid(f.geom)` on every bbox-matched polygon, and nearly every county flood layer held INVALID huge polygons (Marion: a single 550,457-vertex invalid polygon; jefferson 1.6M, desoto 1.8M). ST_MakeValid on an invalid half-million-vertex polygon is ~26s; on the same polygon once it is valid, ~2.4s; skipped entirely (valid, guarded by ST_IsValid), ~11ms. The fix was three-layered: (1) a `(co_no, parcel_id)` index on the 10.7M-row `parcels_staging` (the county-scan for the geom lookup was 7.2s → 1ms — verify with EXPLAIN, `Rows Removed by Filter` is the tell); (2) validate every invalid flood geom in place, statewide, ONCE; (3) guard the resolver's ST_MakeValid with ST_IsValid so valid geoms skip the rebuild. Result: Marion 27.7s→4.9s, Volusia 14.4s→7.3s. The cost of a per-call geometry repair scales with (polygon vertices × query frequency) — it belongs at the data-load boundary, asserted, not in the read path. And: don't fix a list of suspects blindly; profile to the one that dominates (the other three contributors here were <3s combined).

> **Standing rule: never assert a co_no from memory — resolve it from `county_registry`. DOR county codes are alphabetical from 11 (Alachua=11 … Calhoun=17, Citrus=19 …), NOT positional or recalled.** This has bitten the project three times: a relayed benchmark used co_no 64 (Putnam) mislabeled as Volusia (74); a Pinellas 52-vs-62 mix-up; and a land_use test that assumed Citrus=17 (it is 19; 17 is Calhoun) and would have read the wrong county's layer had verification not caught it. Each time the same fix: `SELECT dor_county_no FROM county_registry WHERE county_name=…`, never a literal from recall.

> **Standing rule: a "value that means absence" (a sentinel) must be detected and rendered as what it MEANS, not what it looks like — this is the single most repeated defect in this data.** It has appeared at least six times: `999`/`INCORPORATED` zoning (the county saying "not my jurisdiction"), `-9999` in `static_bfe`, `cleanup_status: null` (a closed site with no recorded remediation), an opaque numeric permit `STATUS` with no crosswalk, "closeout not recorded", and the NWI **`Estuarine and Marine Deepwater`** + **`Lake`** categories in `fl_nwi_wetlands` — 15.3M acres of OPEN WATER (ocean, estuary, lakes) inside a layer named "wetlands". This last is the sentinel's sibling: not a value that means *absence*, but a **category that looks like the thing you are measuring and is not** — counting it would report ~60% of Florida as wetland, indefensible on first inspection. Same fix: read what the category MEANS before counting it (a type/acreage distribution surfaces it — Deepwater alone is 14.0M ac). Each looks like a real value and silently corrupts the fact — or the metric. The fix is always the same three steps: (1) find the sentinel (a code-value distribution surfaces it — a high-frequency value that is not a member of the concept's category set: 999, 0, N/A, INC, blank), (2) map it to the absence it denotes, (3) render the honest gap (often naming what the sentinel points AT — the governing municipality, the missing datum). A sentinel seeded silently (e.g. auto-discovery that sets a code column but not the layer's `not_mine_values`) reintroduces the defect at the scale of the seed — so sentinel detection is part of wiring a layer, not a follow-up.

> **Standing rule: a fabrication can be GENERATION, not exposure — and then removing data cannot fix it; only a mechanical gate at the response boundary can.** The elevation lie recurred a SEVENTH time ("Ground elevation: 15.9 ft, USGS 3DEP lidar-derived, ±0.96 ft at 95% confidence") AFTER the payload was verified clean — `land: {}`, `groundElevation` withheld with `value:null`. The number 15.9 was in NO field; the model INVENTED it (plus the source and the error bar) over a field that explicitly says the value is withheld and why. Three defenses had failed on this exact string: two prompt guards and a withheld fact. The lesson: payload hygiene (the boundary rule below) is necessary but NOT sufficient — it stops *exposure* of a real-but-unrenderable value; it does nothing against *generation* of a value that was never there. The fix is a post-generation tourniquet at the roz response boundary: `redactFabricatedPrecision` drops any sentence carrying lidar-accuracy vocabulary (`3DEP`, `lidar`, `± N ft`, `95% confidence`, `NVA`/`VVA`) — terms present in zero honest outputs statewide — and substitutes the honest withheld statement, logging every trigger to `roz_fabrication_block`. Zero false positives by construction. The durable general form (numeric-provenance validation: every rendered number must trace to a payload value) is the follow-up; the vocabulary filter is the tourniquet. Instruction is not a control surface against a determined generator; a mechanical gate the model cannot talk past is.

> **Standing rule: the PAYLOAD is the security boundary, not the render. A guard at the React page protects only the page; a payload consumer reads the raw keys directly and always will.** The elevation fabrication recurred a SIXTH time after the withheld-fact fix: `groundElevation` (value_withheld) was built and the page wired to it, but the bare `land.elevationM`/`land.elevationFt` were left in the payload beside it — and Roz, which reads the payload, took `land.elevationFt` and emitted "USGS 3DEP … 0.64 ft / 0.96 ft at 95% confidence" verbatim again. A render guard is invisible to every non-page consumer. Every superseded key left as "cleanup later" (the flat `flood`/`marineImprovements` shapes, an inline SELECT that still emits) is a LIVE fabrication surface, not debt. When a fact block supersedes a raw key, the raw key must be DELETED FROM THE PAYLOAD in the same change — verified by a Tier-2 predicate (`payload-carries-no-superseded-fabrication-keys`), not by reading the page. Represent each withheld/fact-shaped value exactly ONCE in the payload.

> **Standing rule: two payload keys answering the same question is a live hazard, not tidiness — remove the shadow, don't deprecate it, and close the CLASS with a duplicate-representation check.** The pattern is now four for four: `land.elevationFt` beside `groundElevation` (how the elevation lie recurred a 6th time — Roz read the wrong one); the flat `flood` shape beside `floodBlock`; `marineImprovements` beside `marineBlock`; and a legacy `volusia_zoning`/FLU-backed top-level `zoning` key beside the statewide `zoningFacts` (Volusia-only — on a Miami-Dade parcel it was `{}`, which a consumer could read as "no zoning"; on Volusia it gave a raw `zoncode` without the fact's definition/municipal-precedence logic). A consumer taking the wrong one is invisible until someone reads a report, so a superseded key that still emits is a fabrication surface — a render guard can't see it, the payload is the boundary. The general defense is cheaper than finding the fifth by reading a report: `payload-carries-no-superseded-fabrication-keys` is now a two-sided duplicate-representation closure — every retired shadow must be ABSENT and its canonical key PRESENT (so an over-removal that drops both also goes red), and a new pair is added the instant it's retired. Brownfield was the counter-example done right: repointed in place (one key, statewide value), never duplicated.

> **Standing gate: a layer joins the ENCUMBRANCE metric only if it is a DESIGNATION or BOUNDARY arising from a specific per-parcel DETERMINATION — never a modelled index or a blanket classification.** Qualifying: GWCA (Ch.62-524 well bar), brownfield area, institutional control, SWAPP source-water-protection area, historic-district membership, a mine boundary — each is a recorded determination about *this* land. Disqualified, always: a MODELLED INDEX (DRASTIC aquifer vulnerability — a scored rating, not a determination) and a BLANKET CLASSIFICATION that covers essentially the whole state (FGS geology, geomorphology districts/provinces — geomorphology is **100% of Florida**, so including it would define the metric to 100% by construction). The test is not "is it a hazard?" but "is it a determination about this parcel, or a property of everywhere?" Geology and DRASTIC are real report content (rendered with their modelled/regional caveats) — they are just never *encumbrance-metric* members. A modelled or blanket layer that slips into the metric makes the headline number indefensible.

> **Standing gate: the two product metrics are SEPARATE in every context — report, deck, marketing — and never conflated.** Metric 1 is CONTAMINATION (a parcel with a contamination finding); metric 2 is ENCUMBRANCE (a parcel under a recorded designation/restriction). They measure different things and move independently. The moment metric 2 (e.g. 3.87%) is quoted as "contaminated," the claim is indefensible and the correction is public. Label every metric with which one it is; never let an encumbrance percentage be read as a contamination percentage.

> **Standing rule: a value-gating test must assert the VALUE on the key, not the label — a test that still passes when the gated value is null everywhere is testing presentation, not correctness.** The transactions block classified sales correctly (market vs non-market) but put the money on the wrong key: `sale_price` was null on every conveyance including the qualified $2.125M warranty deed, with the amount sitting on `consideration` for all of them. Nine render controls passed — because they asserted the *labels* ("$2,125,000", "consideration") not the *values on the keys*. The one sale that is a market price was presented identically to the four that aren't — the exact failure the gating exists to prevent, inverted, and invisible to a green suite. The fix added a control asserting `sale_price != null` on at least one conveyance. When a test's subject is "the right number reaches the right field", assert the number at the field; a label assertion passes on an empty field.

> **Standing rule: "done" means the SERVED PATH reaches it — not that a function exists, a table is populated, or a socket works.** Backlog item 3 ("GWCA + institutional controls wired") was marked done on evidence "953 Volusia parcels, 33 IC polygons": `fdep_gwca` (376 zones), `gwca_parcel_match` (195,358 matches), and `get_parcel_restrictions` all existed and worked — but `get_pir_report` never *called* `get_parcel_restrictions`, and the one path that did reach the payload (`get_parcel_env_findings` via `get_parcel_disclosures`) is co_no-74-gated. So the Ch.62-524 potable-well prohibition — a criminal-penalty finding — reached no buyer outside Volusia while the backlog said finished. This is the THIRD instance of the class (a flood block rendering a dash; a disclosure sitting unrendered in the payload; GWCA orphaned). The test is mechanical and is the CALL-GRAPH transitive closure, not a direct grep: `get_parcel_env_findings` is reached only via a wrapper, so a direct scan of `get_pir_report` would wrongly call it orphaned. Auditing the 5 done items whose evidence named Volusia found exactly one more orphan (this one); the other four are correctly served on their own paths (Roz route `find_parcel` coverageNote; `/api/agent` → `agent_claim_*`; a `parcels_staging` index). Converted from a thing-we-remember into a thing-that-fires: predicate `served-path-reaches-every-pir-fact-resolver` computes the closure from `get_pir_report` and goes RED if any `get_parcel_*_facts` (or the named block/restriction resolvers) exists but isn't reached — auto-including any future `*_facts` resolver, fail-closed.

> **Standing rule: a "no-op" refactor must be proven no-op on BOTH branches, and the proof must use a case where the branches can actually diverge.** Repointing `get_pir_report`'s values block to `get_parcel_values_facts` was asserted equal on Volusia 744403020120 (CAMA branch, all 4 fields exact) — but the CAMA and NAL branches compute `improvement_value` *differently*, and the first NAL test parcel (Marion 2572-010-003) had **$0 special features**, so it could not distinguish the bug from correct code. The resolver was silently omitting the `special_feature_value` subtraction the report performs; only a parcel with **$45.6M in special features** (Marion 21068-001-01: $136.58M wrong vs $90.95M right) made the test bite. Equality on the branch that *can't* diverge proves nothing about the branch that can. This is the negative-control principle applied to a refactor: pick the input that would expose the difference if it exists, not the input that's convenient. Assert-equality-before-swap is only as strong as the discriminating power of the case you assert on.
...
| GWCA "wired = done" reached one county `[2026-07-31]` | backlog item 3 marked done — the Ch.62-524 potable-well prohibition (criminal penalty) is served in the report | `get_pir_report` **never called** `get_parcel_restrictions`; the only path carrying GWCA to the payload (`get_parcel_env_findings` via `get_parcel_disclosures`) is **co_no-74-gated**, so every county but Volusia got nothing — while `fdep_gwca` (376 zones), `gwca_parcel_match` (195,358), and the resolver all existed and worked. A directive asserted "we hold zero tables"; a query showed 4. Fixed: `landRestrictions` wired statewide (verified Alachua 02514-007-000) | call-graph transitive closure from `get_pir_report` (a wrapper still counts). Audited all 5 done+Volusia items; 1 orphan (this), 4 correctly served elsewhere. Standing rule + predicate `served-path-reaches-every-pir-fact-resolver` added so it fires, not remembered |
| values refactor "no-op" nearly shipped a $45.6M error `[2026-07-30]` | `get_parcel_values_facts` == `get_pir_report` inline coalesce (asserted on Volusia CAMA parcel, 4/4 exact) | the resolver **omitted the `special_feature_value` subtraction** on the improvement branch; the CAMA branch (Volusia) can't expose it, and the first NAL parcel (Marion 2572) had **$0 special** → falsely equal. A $45.6M-special parcel (Marion 21068-001-01) showed $136,581,499 wrong vs $90,948,950 right | had to *go find* a discriminating parcel. Standing rule added above: a no-op proof must run on a case where the branches can diverge — equality on a non-discriminating input proves nothing |
