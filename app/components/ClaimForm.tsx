...
export default function ClaimForm({ slug, licenseNumber, displayName }: Props) {
  ...
  if (step === 'submitted') {
    return (
      <div className="rounded-xl p-8 text-center" style={{ background: 'var(--color-white)', border: '1px solid var(--color-light-gray)' }}>
  ...
  return (
    <div className="rounded-xl p-6" style={{ background: 'var(--color-white)', border: '1px solid var(--color-light-gray)' }}>
      {step === 'verify' && (
        <>
          ...
          <input
            ...
            style={{ border: '1px solid var(--color-light-gray)', background: 'var(--color-cream)', color: 'var(--color-ink)', outline: 'none' }}
          />
      ...
      {step === 'details' && (
        <>
          ...
          <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--color-ink)' }}>Your Name</label>
          <input
            type="text"
            value={name}
            ...
            style={{ border: '1px solid var(--color-light-gray)', background: 'var(--color-cream)', color: 'var(--color-ink)', outline: 'none' }}
          />
          <label className="block text-xs font-semibold mb-1" style={{ color: 'var(--color-ink)' }}>Email Address</label>
          <input
            type="email"
            value={email}
          ...
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 rounded-lg text-sm mb-4"
            style={{ border: '1px solid var(--color-light-gray)', background: 'var(--color-cream)', color: 'var(--color-ink)', outline: 'none', resize: 'vertical' }}
          />
          {error && <p className="text-xs mb-4" style={{ color: '#c0392b' }}>{error}</p>}
          <button
            ...
            style={{ background: loading ? 'var(--color-sage)' : 'var(--color-bronze)', color: 'white', border: 'none', cursor: loading ? 'not-allowed' : 'pointer' }}
          >
