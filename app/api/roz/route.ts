...
const TOOLS: Anthropic.Tool[] = [
  ...
  // The description used to promise "complaint_count, bond/insurance". Those fields are empty on every
  // row we hold (measured 2026-09-26: 0 of 114,104), and a model told it has complaint counts that is
  // handed an empty one says "no complaints on record" — a clearance about a named business nobody
  // checked. It also led with "electrical", a board whose file we do not hold (1 row statewide).
  { name: 'find_contractors', description: 'Find Florida construction contractors in the state DBPR construction licence file, by trade and county (Volusia default). trade e.g. "roofing", "plumbing", "pool", "general". Returns each licence\'s number, status and expiry as recorded in that file, and nothing else about the contractor. We hold NO complaint, discipline, bond, insurance or workers\' comp data: never say or imply a contractor has no complaints, a clean record, or is bonded or insured — say we do not hold it and point to myfloridalicense.com. Electrical contractors are licensed by a separate board whose file we do not hold, so an electrical search returning few or no results is a gap in OUR data, never a statement about how many electricians there are. City is a soft preference only — the DBPR city is the business address, not service area, so search by county.',
