...
export async function GET(req: NextRequest) {
  ...
  if (!allowed) {
    return NextResponse.json({ error: 'Rate limit exceeded.' }, { status: 429, headers: { 'X-RateLimit-Limit': '30', 'X-RateLimit-Remaining': '0', 'X-RateLimit-Reset': String(Math.ceil(resetAt / 1000)), 'Retry-After': String(Math.ceil((resetAt - Date.now()) / 1000)) } })
  }
  ...
  try {
    ...
    return NextResponse.json({ contractors: contractors, count: contractors.length }, { headers: { 'X-RateLimit-Limit': '30', 'X-RateLimit-Remaining': String(remaining), 'X-RateLimit-Reset': String(Math.ceil(resetAt / 1000)), 'Cache-Control': 'public, s-maxage=30, stale-while-revalidate=60' } })
  } catch (err) {
    const detail = err instanceof Error ? err.message : String(err)
    console.error('[/api/contractors]', detail)
    return NextResponse.json({ error: 'Database error', detail: detail }, { status: 500 })
  }
}